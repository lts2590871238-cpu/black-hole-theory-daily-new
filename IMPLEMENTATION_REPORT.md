...
13. arXiv 抓取改用每页 100 条、最多 10 页和 60 秒读取超时；后续分页超时会保留已成功页面。arXiv 或 INSPIRE 单源不可用时任务降级继续，只有两个来源同时不可用才停止部署并保留上一版站点。
...
## 2026-07-26 arXiv 超时容错验收

针对 arXiv API 首次请求超时导致每日任务直接退出的问题，现已完成分页降载、延长读取超时、后续分页部分成功保留，以及 arXiv/INSPIRE 单数据源故障降级。两个来源同时不可用时仍会停止发布并保留上一版站点，防止空内容覆盖线上页面。

```text
ruff check .       通过
mypy src           通过（18 个源码文件，无问题）
pytest -q          通过（37 passed）
demo --fixtures    通过（8 篇明确标记的虚构示例）
validate           通过
```

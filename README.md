...
正式更新缺少 `OPENAI_API_KEY` 时会明确失败，不会发布未经评分或翻译的论文。arXiv 或 INSPIRE 任一数据源暂时不可用时，另一数据源仍可继续，运行报告会标记降级；两者同时不可用时保留上一版站点。arXiv 请求使用较小分页、较长读取超时和自动重试，后续页面超时时会保留已成功抓取的页面。若本次所有模型调用均失败，上一版 `dist/` 会保留。
